...
export default function ContentCalendarPage() {
  ...
  const ganttData = useMemo(() => {
    ...
    // Content bars (multi-day tasks, excluding weekly Friday)
    const contentBars = [];
    CONTENT_TASKS.forEach((task) => {
      task.ranges.forEach(([sm, sd, em, ed]) => {
        const clamped = clampToMonth(sm, sd, em, ed);
    ...
    // Friday markers within this month
    const fridayMarkers = [];
    for (let d = 1; d <= daysInMonth; d++) {
      const date = new Date(year, month, d);
      if (date.getDay() === 5) {
        fridayMarkers.push({ day: d });
      }
    }
    ...
    <>
      ...
      {viewMode === "gantt" && (
        <div className={styles.ganttContainer}>
          ...
          {/* Content Tasks Group */}
          {(ganttData.contentBars.length > 0 || ganttData.fridayMarkers.length > 0) && (
            <div className={styles.ganttGroup}>
              <div className={styles.ganttGroupHeader}>
                <svg className={styles.legendIcon} viewBox="0 0 24 24" fill="none" stroke="#06b6d4" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                Content Tasks
              </div>
              {ganttData.contentBars.map((bar, i) => {
                const phaseClass = styles[`ganttBarPhase${bar.phase.charAt(0).toUpperCase() + bar.phase.slice(1)}`] || "";
                return (
                  <div key={`content-${i}`} className={styles.ganttRow}>
                    <div className={styles.ganttLabelCol} title={bar.name}>
                      <span className={styles.ganttLabelText}>{bar.name}</span>
                    </div>
                    <div className={styles.ganttDayCols} style={{ gridTemplateColumns: `repeat(${ganttData.daysInMonth}, minmax(28px, 1fr))` }}>
                      {Array.from({ length: ganttData.daysInMonth }, (_, j) => {
                        ...
                        const isStart = d === bar.startDay;
                        const isEnd = d === bar.endDay;
                        let barCls = styles.ganttBar + " " + styles.ganttBarContent + " " + phaseClass;
                        if (isStart) barCls += " " + styles.ganttBarStart;
                        if (isEnd) barCls += " " + styles.ganttBarEnd;
                        if (isStart && bar.startsBeforeMonth) barCls += " " + styles.ganttBarClipLeft;
                        if (isEnd && bar.endsAfterMonth) barCls += " " + styles.ganttBarClipRight;
              ...
              {ganttData.fridayMarkers.length > 0 && (
                <div className={styles.ganttRow}>
                  <div className={styles.ganttLabelCol}>
                    <span className={styles.ganttLabelText}>Weekly Prep</span>
                  </div>
                  <div className={styles.ganttDayCols} style={{ gridTemplateColumns: `repeat(${ganttData.daysInMonth}, minmax(28px, 1fr))` }}>
                    {Array.from({ length: ganttData.daysInMonth }, (_, j) => {
                      const d = j + 1;
                      const isFriday = ganttData.fridayMarkers.some((m) => m.day === d);
                      const dayKey = `${year}-${String(month + 1).padStart(2, "0")}-${String(d).padStart(2, "0")}`;
                      return (
                        <div key={d} className={styles.ganttCell} onClick={isFriday ? () => selectDay(dayKey) : undefined}>
                          {isFriday && <div className={`${styles.ganttMarker} ${styles.ganttMarkerContent}`} title="Prepare Sunday creatives">F</div>}
                        </div>
                      );
                    })}
                  </div>
