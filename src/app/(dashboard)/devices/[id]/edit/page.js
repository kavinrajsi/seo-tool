...
import {
  ...
  BatteryIcon, WifiIcon, PaletteIcon, LoaderIcon, ChevronDownIcon,
} from "lucide-react";
...
export default function EditDevice({ params }) {
  ...
  useEffect(() => {
    Promise.all([
      supabase.from("device_vendors").select("name").order("name"),
      supabase.from("devices").select("*").eq("id", id).single(),
    ]).then(([vendorRes, deviceRes]) => {
      if (vendorRes.data) setVendors(vendorRes.data.map((v) => v.name));
      if (deviceRes.data) {
  ...
  async function checkSerial() {
    ...
    const { data } = await supabase
      .from("devices")
      .select("id, device_id, device_type, model_name")
  ...
  async function handleSubmit(e) {
    ...
    const { error } = await supabase.from("devices").update(row).eq("id", id);
    if (error) {
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 py-4 max-w-3xl">
      <div>
        ...
        <p className="text-muted-foreground mt-1 text-sm">Update device information and specifications.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end gap-3 pt-4 pb-8">
          <button type="button" onClick={() => router.push(`/devices/${id}`)} className="rounded-lg border border-border px-6 py-3 text-sm font-medium hover:bg-muted/30 transition-colors">
            Cancel
          </button>
