...
export async function GET(request) {
  ...
  const redirectUrl = new URL("/dashboard/search-console", baseUrl);

  if (error) {
    redirectUrl.searchParams.set("gsc_error", error);
    return NextResponse.redirect(redirectUrl);
  }

  if (!code || !state) {
    redirectUrl.searchParams.set("gsc_error", "missing_params");
    return NextResponse.redirect(redirectUrl);
  }
  ...
  if (!user || user.id !== userId) {
    redirectUrl.searchParams.set("gsc_error", "auth_mismatch");
    return NextResponse.redirect(redirectUrl);
  }
  ...
  if (!tokenRes.ok) {
    redirectUrl.searchParams.set("gsc_error", "token_exchange_failed");
    return NextResponse.redirect(redirectUrl);
  }
  ...
  if (!access_token || !refresh_token) {
    redirectUrl.searchParams.set("gsc_error", "no_refresh_token");
    return NextResponse.redirect(redirectUrl);
  }
  ...
  let deleteQuery = admin.from("gsc_connections").delete().eq("user_id", user.id);
  if (parsedProjectId) {
  ...
  if (dbError) {
    redirectUrl.searchParams.set("gsc_error", "db_save_failed");
    return NextResponse.redirect(redirectUrl);
  }
