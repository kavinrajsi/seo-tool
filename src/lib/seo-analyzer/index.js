...
import { CDN_DOMAINS, DEPRECATED_TAGS } from "./constants";
import { safeFetch } from "./fetch";
import { extractKeywords } from "./keywords";
import { check, analyzeLlmsTxt } from "./checks";
import { calculateScores } from "./scoring";

/**
 * Analyse a URL for SEO issues. Returns the full analysis object.
 * @param {string} url — raw URL string (may or may not include protocol)
 */
export async function analyzeUrl(url) {
  ...
  ldJsonScripts.each((_, el) => {
    try {
      ...
      logError("seo-analyzer/structuredData", err);
    }
  });
  ...
  if (spfResult.status === "fulfilled" && spfResult.value) {
    try {
      ...
      logError("seo-analyzer/spfCheck", err);
    }
  }
  ...
  for (const dr of dirResults) {
    if (dr.status === "fulfilled" && dr.value && dr.value.ok) {
      try {
        ...
        logError("seo-analyzer/directoryListing", err);
      }
    }
  }
  ...
  if (safeBrowsingResult.status === "fulfilled" && safeBrowsingResult.value) {
    try {
      ...
      logError("seo-analyzer/safeBrowsing", err);
  ...
  if (wwwRedirectResult.status === "fulfilled" && wwwRedirectResult.value) {
    try {
      ...
      logError("seo-analyzer/wwwRedirect", err);
  ...
  // Scoring
  // -----------------------------------------------------------------------
  const { score, category_scores } = calculateScores(checks);
