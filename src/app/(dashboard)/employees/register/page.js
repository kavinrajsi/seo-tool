...
function Input({ value, onChange, error, disabled, placeholder, type = "text", className = "" }) {
  return (
    <input
      ...
      className={`w-full rounded-md border px-3 py-2 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/40 disabled:opacity-40 disabled:bg-muted/30 ${error ? "border-red-400" : "border-border"} bg-background ${className}`}
    />
  );
}
...
function Select({ value, onChange, error, options, placeholder }) {
  return (
    <select
      value={value}
      onChange={onChange}
      className={`w-full rounded-md border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 bg-background ${error ? "border-red-400" : "border-border"} ${!value ? "text-muted-foreground" : ""}`}
    >
