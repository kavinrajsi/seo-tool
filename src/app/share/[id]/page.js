...
const ANALYSIS_CONFIG = [
  ...
  { key: "socialMediaMetaTags", title: "Social Media Meta Tags", description: "Social media meta tags." },
  ...
  { key: "llmsTxt", title: "llms.txt File", description: "Checks for /llms.txt file for LLM discoverability." },
];
...
function renderCardContent(key, result, reportUrl, allResults) {
  ...
  switch (key) {
    case "title":
      return result.title ? (<div><strong>{result.title}</strong><br /><small>{result.length} characters</small></div>) : null;
    case "metaDescription":
      return result.description ? (<div><em>{result.description}</em><br /><small>{result.length} characters</small></div>) : null;
    case "h1":
      return result.h1Texts && result.h1Texts.length > 0 ? (<div>{result.h1Texts.map((text, i) => (<div key={i}><strong>H1 #{i + 1}:</strong> {text}</div>))}</div>) : null;
    case "headingHierarchy":
      return <HeadingTree headings={result.headings} />;
    case "contentAnalysis":
      return result.keywords ? (<KeywordAnalysis wordCount={result.wordCount} keywords={result.keywords} twoWordPhrases={result.twoWordPhrases} threeWordPhrases={result.threeWordPhrases} fourWordPhrases={result.fourWordPhrases} />) : null;
    ...
    case "internalLinks":
      return result.links && result.links.length > 0 ? (<LinkList links={result.links} baseUrl={reportUrl.replace(/\/$/, "")} showAnchor={true} />) : null;
    case "externalLinks":
      return result.links && result.links.length > 0 ? (<LinkList links={result.links} showAnchor={true} />) : null;
    case "openGraph":
    case "twitterCards":
    ...
    case "socialMediaMetaTags": {
      ...
      const hasOg = ogResult?.tags && Object.keys(ogResult.tags).length > 0;
      const hasTw = twResult?.tags && Object.keys(twResult.tags).length > 0;
      if (!hasOg && !hasTw) return null;
      return (
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          {hasOg && (<div><span style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--color-indigo-700)", display: "block", marginBottom: "6px" }}>Open Graph Tags ({Object.keys(ogResult.tags).length})</span><div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>{Object.entries(ogResult.tags).map(([tag, value]) => (<div key={tag} style={{ fontSize: "0.78rem", background: "var(--color-slate-50)", padding: "5px 10px", borderRadius: "var(--radius-sm)", wordBreak: "break-all" }}><strong style={{ color: "var(--color-indigo-700)" }}>{tag}</strong><span style={{ color: "var(--color-slate-600)", marginLeft: "8px" }}>{value || "(empty)"}</span></div>))}</div></div>)}
          {hasTw && (<div><span style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--color-indigo-700)", display: "block", marginBottom: "6px" }}>Twitter Card Tags ({Object.keys(twResult.tags).length})</span><div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>{Object.entries(twResult.tags).map(([tag, value]) => (<div key={tag} style={{ fontSize: "0.78rem", background: "var(--color-slate-50)", padding: "5px 10px", borderRadius: "var(--radius-sm)", wordBreak: "break-all" }}><strong style={{ color: "var(--color-indigo-700)" }}>{tag}</strong><span style={{ color: "var(--color-slate-600)", marginLeft: "8px" }}>{value || "(empty)"}</span></div>))}</div></div>)}
        </div>
      );
    }
    ...
    case "llmsTxt":
      return (
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          ...
          {result.title && (
            <div style={{ fontSize: "0.82rem" }}><strong>Title:</strong> {result.title}</div>
          )}
          {result.description && (
            <div style={{ fontSize: "0.82rem", color: "var(--color-slate-600)" }}><strong>Description:</strong> {result.description}</div>
          )}
...
export default function ShareReportPage({ params }) {
  const { id } = use(params);
  ...
  if (loading) return <div className={styles.container}><p className={styles.loading}>Loading report...</p></div>;

  if (error) {
    return (
      <div className={styles.container}>
        <div className={styles.error}>{error}</div>
        <Link href="/" className={styles.ctaLink}>Analyze your own URL</Link>
      </div>
    );
  }
      ...
      <div className={styles.content}>
        <div className={styles.heroCard}>
          ...
          <div className={styles.heroRight}>
            <h1 className={styles.heroTitle}>SEO Analysis Report</h1>
            ...
            <div className={styles.severityDots}>
              {report.fail_count > 0 && <span className={styles.dotFail}>{report.fail_count} Critical</span>}
              {report.warning_count > 0 && <span className={styles.dotWarning}>{report.warning_count} Warnings</span>}
              {report.pass_count > 0 && <span className={styles.dotPass}>{report.pass_count} Passed</span>}
            </div>
          </div>
        </div>
        ...
        {failCards.length > 0 && (
          <div className={styles.severitySection}>
            <h3 className={`${styles.sectionHeader} ${styles.sectionHeaderFail}`}>Critical Issues ({failCards.length})</h3>
            {failCards.map((card) => { runningIndex++; return (
              <AnalysisCard key={card.key} title={card.title} description={card.description} score={card.result?.score || "fail"} issues={card.result?.issues} recommendations={card.result?.recommendations} index={runningIndex} defaultExpanded={true}>
        ...
        {warningCards.length > 0 && (
          <div className={styles.severitySection}>
            <h3 className={`${styles.sectionHeader} ${styles.sectionHeaderWarning}`}>Warnings ({warningCards.length})</h3>
            {warningCards.map((card) => { runningIndex++; return (
              <AnalysisCard key={card.key} title={card.title} description={card.description} score={card.result?.score || "warning"} issues={card.result?.issues} recommendations={card.result?.recommendations} index={runningIndex} defaultExpanded={true}>
                {renderCardContent(card.key, card.result, report.url, results)}
              </AnalysisCard>
            ); })}
          </div>
        ...
        {passCards.length > 0 && (
          <div className={styles.severitySection}>
            <button className={`${styles.sectionHeader} ${styles.sectionHeaderPass} ${styles.collapseToggle}`} onClick={() => setPassedExpanded(!passedExpanded)} type="button" aria-expanded={passedExpanded}>
              Passed Checks ({passCards.length})
              <span className={`${styles.toggleChevron} ${passedExpanded ? styles.toggleChevronOpen : ""}`}>&#8250;</span>
            </button>
            {passedExpanded && passCards.map((card) => { runningIndex++; return (
              <AnalysisCard key={card.key} title={card.title} description={card.description} score={card.result?.score || "pass"} issues={card.result?.issues} recommendations={card.result?.recommendations} index={runningIndex} defaultExpanded={false}>
                {renderCardContent(card.key, card.result, report.url, results)}
              </AnalysisCard>
            ); })}
          </div>
