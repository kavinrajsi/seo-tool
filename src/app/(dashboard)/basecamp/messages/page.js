...
export default function BasecampMessages() {
  ...
  async function loadEvents() {
    setLoading(true);
    try {
      const res = await fetch("/api/basecamp/events?filter=messages&limit=500");
  ...
  const filtered = events.filter((e) => {
    ...
    if (typeFilter === "messages" && !e.event_kind?.includes("message")) return false;
    if (typeFilter === "comments" && !e.event_kind?.includes("comment")) return false;
    return true;
  });

  const msgCount = events.filter((e) => e.event_kind?.includes("message")).length;
  const commentCount = events.filter((e) => e.event_kind?.includes("comment")).length;
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 py-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight flex items-center gap-2">
            <MessageSquareIcon size={24} className="text-purple-400" />
            Messages & Comments
          </h1>
          <p className="text-muted-foreground mt-1">{events.length} message events from Basecamp</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="rounded-xl border border-border bg-card p-4 text-center">
          <p className="text-2xl font-bold text-blue-400">{msgCount}</p>
          <p className="text-xs text-muted-foreground mt-1">Messages</p>
        </div>
        <div className="rounded-xl border border-border bg-card p-4 text-center">
          <p className="text-2xl font-bold text-amber-400">{commentCount}</p>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Search messages..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full rounded-md border border-border bg-background pl-9 pr-3 py-2 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/60" />
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-muted-foreground gap-2">
          ...
          <p className="text-sm">{events.length === 0 ? "No message events yet." : "No matching messages."}</p>
        </div>
      ) : (
        <div className="rounded-xl border border-border bg-card overflow-hidden">
          {filtered.map((event, i) => {
            const isComment = event.event_kind?.includes("comment");
            return (
              <div key={event.id} onClick={() => setSelectedEvent(event)} className={`flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-muted/20 transition-colors ${i < filtered.length - 1 ? "border-b border-border/50" : ""}`}>
                <MessageSquareIcon size={16} className={isComment ? "text-amber-400" : "text-purple-400"} />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate">{event.recording_title || "Untitled"}</p>
                  <div className="flex items-center gap-3 mt-0.5 text-xs text-muted-foreground">
                    <span className={`text-[10px] px-1.5 py-0.5 rounded ${isComment ? "bg-amber-500/10 text-amber-400" : "bg-purple-500/10 text-purple-400"}`}>{isComment ? "Comment" : "Message"}</span>
